...
public class GraphicManager {
    ...
    for (int i = 0; i < string.length(); i++) {
        ...
        // 対応していない文字、画像がない文字はスキップ
        if (img == null || img.getBufferedImage() == null) {
            continue;
        }

        java.awt.image.BufferedImage bi = img.getBufferedImage();
        screenGraphic.drawImage(bi, nowPositionX, y, bi.getWidth(), bi.getHeight(), null);
        nowPositionX += bi.getWidth();
    }
}
